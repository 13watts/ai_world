...
const feedRefreshMinutes = Number.parseInt(process.env.AI_WORLD_FEED_REFRESH_MINUTES ?? "30", 10);
const feedRefreshIntervalMs = Number.isFinite(feedRefreshMinutes) && feedRefreshMinutes > 0
    ? feedRefreshMinutes * 60 * 1000
    : 30 * 60 * 1000;
let feedRefreshRunning = false;
function refreshFeedsInBackground(reason) {
