...
export function classifyModalitiesFromText(input, defaults = []) {
    const text = normalize(input);
    const scores = Object.fromEntries(modalities.map((modality) => [modality.slug, 0]));
    const rationale = [];
    ...
    if (modalitySlugs.length === 0)
        modalitySlugs = defaults.length > 0 ? [...defaults] : ['research-ml'];
    return { modalitySlugs: [...new Set(modalitySlugs)], scores, rationale: [...new Set(rationale)].slice(0, 12) };
}
export function classifyItemModalities(source, title, summary, categories = []) {
