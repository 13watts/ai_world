...
function logFileFor(kind, date = new Date()) {
    const parts = dateParts(date);
    return path.join(logRoot, kind, parts.year, parts.month, `${parts.stamp}.jsonl`);
}
function sanitizeEvent(event) {
    return Object.fromEntries(Object.entries({ ts: new Date().toISOString(), ...event }).filter(([, value]) => value !== undefined));
}
export async function appendActivityLog(kind, event) {
...
export function logPageAccess(req, res, next) {
...
export async function logFeedCrawlEvent(event) {
...
export async function getRecentLogEvents(kind, limit = 200) {
