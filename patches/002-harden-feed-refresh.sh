...
# Replace feed refresh service with a hardened implementation:
# - fetches feed text directly so HTTP status failures are clear
# - supports fallbackUrls per feed
# - sanitizes malformed XML ampersands before parsing
# - honors enabled: false for temporarily disabled feeds
...
echo "Patched feed refresh service."
